...
for i in range(len(M)):
    ...
    for j in range(R):
        ...
        for run in range(n_runs):
            ant_inc = get_next_point_oneEI_inc(train_x, train_y,
                                               best_value, bounds,
                                               num_samples=N_ref,
...
for i in range(n_runs):
    mean_point = error_point[:, :, i].mean(dim=0)
    ...
    plt.savefig("../figures/SelfDefinedFunction/MLIncEIrate{}obs{}relsBeta{}.eps".format(num_obs, R, rate), format='eps')
    plt.show()
