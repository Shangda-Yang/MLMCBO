...
r = 10
for j in range(r):
    best_candidate, best_result, _ = mc_run(0.05, alpha, target, ref=True, **kwargs)
...
print("--- MC starts ---")
for i in range(len(Eps_sl)):
    eps_sl = Eps_sl[i]
    start_time = time.time()
    print("-------------------------")
    print("Eps {}".format(eps_sl))
    print("-------------------------")
    for j in range(R):
...
end_time = time.time()
print("--- MC Time {:.4f} seconds ---".format(end_time - start_time))
...
for i in range(len(Eps_ml)):
    ...
    for j in range(R):
        ...
        print("Realisation {} finished, time {}".format(j, time.time() - start_time))
end_time = time.time()
print("--- MLMC Time {:.4f} seconds ---".format(end_time - start_time))
...
plt.savefig("onestepMSE{}R{}ML{:.4f}andMCrate{:.4f}.eps".format(fun_name, R, rate_ml, rate_sl), format='eps')
# plt.show()
